...
| Carbon-style behavior | go-calendar replacement |
| --- | --- |
| Mutable fluent date | Immutable returned value plus error |
| Implicit current time | `go-clock` plus `calendarclock.Today` |
...
every ambiguous seam, and retain interval algebra in go-temporal.
