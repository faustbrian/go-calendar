...
- `go-clock` owns current time; `go-temporal` owns interval and set algebra.
...
- Compose a civil `Date` with standard-library local time-of-day values or the
  `go-temporal/timeofday` package through an optional adapter.
...
- IANA zone identity and aliases MAY integrate with `go-international` but the
...
- `go-wire`, `go-validation`, and `go-config` adapters.
- `go-clock` helper to obtain today's date in an explicit location.
- `go-temporal` adapters for date periods and sequences without circular core
  dependencies.
- `go-scheduler` MAY consume calendar policies for due-date computation while
...
- Rich interval algebra remains outside core and composes through `go-temporal`.
