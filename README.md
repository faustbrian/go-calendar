...
`go-calendar` provides immutable civil dates, Gregorian calendar arithmetic,
...
- `go-clock` owns current time; [calendarclock](calendarclock) accepts its
  narrow `Now() time.Time` capability.
- `go-temporal` owns interval and set algebra; [calendartemporal](calendartemporal)
